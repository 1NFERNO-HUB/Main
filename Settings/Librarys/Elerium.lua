...
function library:AddWindow(title, options)
	...
	do
		...
		UIS.InputBegan:Connect(function(inputObject)
			...
			elseif inputObject.UserInputType == Enum.UserInputType.Touch then
				...
				if pos.X >= Resizer.AbsolutePosition.X and pos.X <= Resizer.AbsolutePosition.X + Resizer.AbsoluteSize.X and pos.Y >= Resizer.AbsolutePosition.Y and pos.Y <= Resizer.AbsolutePosition.Y + Resizer.AbsoluteSize.Y then
					touchResize = true
					Window.Draggable = false
					spawn(function()
						while touchResize and Resizer.Active and options.can_resize do
							local last = inputObject.Position
							local x = last.X - Window.AbsolutePosition.X
							local y = last.Y - Window.AbsolutePosition.Y
							if x >= options.min_size.X and y >= options.min_size.Y then
								Window.Size = UDim2.new(0, x, 0, y)
							elseif x >= options.min_size.X then
								Window.Size = UDim2.new(0, x, 0, options.min_size.Y)
							elseif y >= options.min_size.Y then
								Window.Size = UDim2.new(0, options.min_size.X, 0, y)
							else
								Window.Size = UDim2.new(0, options.min_size.X, 0, options.min_size.Y)
							end
							RS.Heartbeat:Wait()
		...
		do
			...
			bar.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.Touch then
					touchDrag = true
					Window.Draggable = false
					dragOffset = Vector2.new(input.Position.X - Window.AbsolutePosition.X, input.Position.Y - Window.AbsolutePosition.Y)
				end
			end)
			UIS.InputChanged:Connect(function(input)
				if touchDrag and input.UserInputType == Enum.UserInputType.Touch then
					...
					local sx = math.clamp(newX, 0, math.max(0, workspace.CurrentCamera.ViewportSize.X - Window.AbsoluteSize.X))
					local sy = math.clamp(newY, 0, math.max(0, workspace.CurrentCamera.ViewportSize.Y - Window.AbsoluteSize.Y))
					Window.Position = UDim2.new(0, sx, 0, sy)
				end
			end)
	...
	do
		...
		function window_data:AddTab(tab_name)
			...
			holder.Name = "EleriumHolder"
			holder.BackgroundTransparency = 1
			holder.Size = UDim2.new(1, 0, 1, 0)
			holder.Parent = new_tab
			...
			local function syncContent()
				...
				if h > new_tab.AbsoluteSize.Y then
					if holder.Parent ~= sf then
						holder.Parent = sf
						holder.Size = UDim2.new(1, 0, 0, h)
					end
					sf.Visible = true
				else
					if holder.Parent ~= new_tab then
						holder.Parent = new_tab
						holder.Size = UDim2.new(1, 0, 1, 0)
					end
					sf.Visible = false
			...
			layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(syncContent)
			new_tab:GetPropertyChangedSignal("AbsoluteSize"):Connect(syncContent)
			...
			function tab_data:AddSlider(slider_text, callback, slider_options)
				...
				UIS.InputBegan:Connect(function(inputObject)
					...
					elseif inputObject.UserInputType == Enum.UserInputType.Touch then
						local pos = inputObject.Position
						if pos.X >= slider.AbsolutePosition.X and pos.X <= slider.AbsolutePosition.X + slider.AbsoluteSize.X and pos.Y >= slider.AbsolutePosition.Y and pos.Y <= slider.AbsolutePosition.Y + slider.AbsoluteSize.Y then
							touchHeld = true
							Window.Draggable = false
							spawn(function()
								while touchHeld and (not dropdown_open) do
									local last = inputObject.Position
									local x = (slider.AbsoluteSize.X - (slider.AbsoluteSize.X - ((last.X - slider.AbsolutePosition.X)) + 1)) / slider.AbsoluteSize.X
									local min = 0
									local max = 1
									local size = min
									if x >= min and x <= max then
			...
			function tab_data:AddDropdown(dropdown_name, callback, dropdown_options)
				...
				local open = false
				local multi = dropdown_options.multi == true
				local searchable = dropdown_options.searchable == true
				local items = {}
				local selected = {}
				local searchBox = nil
				if searchable then
					searchBox = Instance.new("TextBox")
					searchBox.Name = "EleriumDropdownSearch"
					...
					searchBox:GetPropertyChangedSignal("Text"):Connect(function()
						...
						local tot = (#objects:GetChildren() - 1) * 20
						if tot < 0 then tot = 0 end
					end)
				end
				dropdown.MouseButton1Click:Connect(function()
					open = not open
					local visibleCount = 0
					for _, c in pairs(objects:GetChildren()) do
						if c:IsA("TextButton") and c.Visible then visibleCount = visibleCount + 1 end
					end
					local len = (visibleCount) * 20
					...
					if open then
						if dropdown_open then return end
						dropdown_open = true
						Resize(box, {Size = UDim2.new(1, 0, 0, len + (searchable and 24 or 0))}, options.tween_time)
					...
					object.MouseButton1Click:Connect(function()
						if not multi then
							...
							if #list == 0 then
								dropdown.Text = "      " .. dropdown_name
							elseif #list == 1 then
								dropdown.Text = "      [ " .. list[1] .. " ]"
							else
								dropdown.Text = "      [ " .. tostring(#list) .. " selected ]"
							end
							pcall(callback, list)
						end
					end)
					...
					if inputObject.UserInputType == Enum.UserInputType.MouseButton1 or inputObject.UserInputType == Enum.UserInputType.Touch then
						...
						spawn(function()
							while Held and Entered1 and (not dropdown_open) do
								local mouse_location = inputObject.Position or gMouse()
								...
								h = x / 100
								s = y / 100
								palette_indicator.Position = UDim2.new(0, math.abs(x - 100) - (palette_indicator.AbsoluteSize.X / 2), 0, math.abs(y - 100) - (palette_indicator.AbsoluteSize.Y / 2))
								update()
								RS.Heartbeat:Wait()
							end
							while Held and Entered2 and (not dropdown_open) do
								...
								local y = ((palette.AbsoluteSize.Y - (mouse_location.Y - palette.AbsolutePosition.Y)) + 1.5)
								v = y / 100
								saturation_indicator.Position = UDim2.new(0, 0, 0, math.abs(y - 100))
								update()
								RS.Heartbeat:Wait()
							end
						end)
			...
			function tab_data:AddColorDropdown(name, callback)
				name = tostring(name or "Color")
				callback = typeof(callback) == "function" and callback or function() end
				local ddOptions = { multi = false, searchable = false }
				local dropdown_data, dropdown = tab_data:AddDropdown(name, function() end, ddOptions)
				local box = dropdown:FindFirstChild("Box")
				local objects = box:FindFirstChild("Objects")
				for _,c in pairs(objects:GetChildren()) do
				...
				cp.Parent = objects
				cp.Position = UDim2.new(0, 5, 0, 5)
				cp.Size = UDim2.new(1, -10, 0, 140)
				local alphaBox = Instance.new("TextBox")
				alphaBox.BackgroundTransparency = 1
				alphaBox.PlaceholderText = "Transparency (0-100)"
				alphaBox.Size = UDim2.new(1, -10, 0, 20)
				alphaBox.Position = UDim2.new(0, 5, 0, 150)
				alphaBox.ClearTextOnFocus = false
				alphaBox.Parent = objects
				alphaBox.ZIndex = objects.ZIndex + 5
				...
				UIS.InputBegan:Connect(function(inputObject)
					if inputObject.UserInputType == Enum.UserInputType.MouseButton1 or inputObject.UserInputType == Enum.UserInputType.Touch then
						...
						spawn(function()
							while Held do
								local pos = inputObject.Position or gMouse()
								if Entered1 then
									local x = ((palette.AbsoluteSize.X - (pos.X - palette.AbsolutePosition.X)) + 1)
									local y = ((palette.AbsoluteSize.Y - (pos.Y - palette.AbsolutePosition.Y)) + 1.5)
									h = x / 100
									s = y / 100
									palette_indicator.Position = UDim2.new(0, math.abs(x - 100) - (palette_indicator.AbsoluteSize.X / 2), 0, math.abs(y - 100) - (palette_indicator.AbsoluteSize.Y / 2))
									update()
				...
				local function apply()
					local col = sample.ImageColor3
					local a = tonumber(alphaBox.Text) or 100
					a = math.clamp(a, 0, 100)
					dropdown.Text = "      [ " .. name .. " ]"
					pcall(callback, col, a/100)
					dropdown_open = false
					Resize(box, {Size = UDim2.new(1, 0, 0, 0)}, options.tween_time)
				end
